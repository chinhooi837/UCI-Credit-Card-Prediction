...
# Install Package & Obtaining data ----------------------------------------
...
#reading data - data is predownloaded from Kaggle and uploaded to github
...
# We also check whether this is an imbalance data set, in this case, it refers to the number of 1s in the target variable default.payment.next.month. 
# There are 22% of labels with 1, which tells us that the data set is slightly imbalanced. 
...
# Next, we will now proceed to exploratory data analysis. We will start by plotting a few visualizations to gain more insights of the data. 
# Do note that before we plot certain graphs, we will be doing some simple pre-processing to improve the readability of the visualizations. 

#functions to convert list of columns names to factor or numeric
...
#changing the name of target variable to target for better visualization readability
...
# Next, we will study the distribution of Age to default payments. We can also see that the distribution of default group is similar to that of the non-default group. 
# Hence, it shows that there is no direct relationship between Age and Default payment.  
...
# We will now study the relationship between limit balance and default payment. 
# The graph shown that the ratio of default clients to the non-default is about the same and distribution is similar. 
...
# We can see that both Payment status and Bill amount are highly correlated to each other, this is understandable as the bill amount are cumulated if it is not paid off. 
# It is also interesting to note that our target variable (default.payment.next.month) is somewhat correlated to payment status (PAY_0 to PAY_6)
...
#To solve the PAY_2 and PAY_5 have new levels 8 error
...
#print AUC value for inspection
cat(paste0("AUC:: ", glm1_perf_auc@y.values), "\n")
...
print(c(accuracy= glm1_acc, cutoff = glm1_cutoff))
...
# this training will take 20 mins depending on the computer specs, hence we can read the rds file with the following line instead.
# Model file is uploaded to Github
rf1<- readRDS("rf1.rds")
...
cat(paste0("AUC:: ", rf1_perf_auc@y.values), "\n")
...
print(c(accuracy= rf1_acc, cutoff = rf1_cutoff))
...
# this training will take 10 mins depending on the computer specs, hence we can read the rds file with the following line instead.
# Model file is uploaded to Github
...
cat(paste0("AUC:: ", xgb1_perf_auc@y.values), "\n")
...
print(c(accuracy= xgb1_acc, cutoff = xgb1_cutoff))
plot(xgb1_perf_acc)
...
# Comparing the performance of these 3 baseline models, we can see that Logistic Regression is not performing well for this kind of slightly imbalanced data set. 
# Hence, for the subsequent study, we will focus on Random Forest and XGBoost. 
...
#code to train the model, it will take more than  10 hours
...
# this training will take more than 10 hours depending on the computer specs, hence we can read the rds file with the following line instead.
# Model file is uploaded to Github
...
cat(paste0("AUC:: ", rf_tune1_perf_auc@y.values), "\n")
...
print(c(accuracy= rf_tune1_acc, cutoff = rf_tune1_cutoff))

plot(rf_tune1_perf_acc)

Model_performance <- bind_rows(Model_performance, 
                               data.frame(Model= "Random Forest Tune",
...
cat(paste0("AUC:: ", xgb_tune1_perf_auc@y.values), "\n")
...
print(c(accuracy= xgb_tune1_acc, cutoff = xgb_tune1_cutoff))
plot(xgb1_perf_acc)
...
# Next, we will select the top 15 features base on the random forest tuned model and retrain it. 
# This is to also test out the performance after removing the less important features which could cause noise to the model. 
...
# )
# 
# rf_tune1 <- train(X_train_fselect,y_train_first,
#                   method = 'rf',
#                   metric = 'Accuracy',
#                   tuneGrid = tune_grid)
...
cat(paste0("AUC:: ", rf_tune1_fselect_perf_auc@y.values), "\n")
...
print(c(accuracy= rf_tune1_fselect_acc, cutoff = rf_tune1_fselect_cutoff))

plot(rf_tune1_fselect_perf_acc)
...
# Lastly, we will select the top 15 features base on the XGB tuned model and retrain it. 
# This is to also test out the performance after removing the less important features which could cause noise to the model.
...
cat(paste0("AUC:: ", xgb_tune1_fselect15_perf_auc@y.values), "\n")
...
print(c(accuracy= xgb_tune1_fselect15_acc, cutoff = xgb_tune1_fselect15_cutoff))
plot(xgb_tune1_fselect15_perf_acc)
...
# Judging from the performances of the Models, both XGBoost and Random Forest have a much better performance than logistic regression. Among which, XGBoost slightly outperforms Random Forest and have a higher AUC. In this case, feature selection does not help in improving the performance, 
# i.e. almost comparable for XGBoost and even worse off performance for Random Forest.
